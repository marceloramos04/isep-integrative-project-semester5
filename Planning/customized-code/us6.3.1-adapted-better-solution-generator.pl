...
surgery(so2,30,45,30).
surgery(so3,35,50,35).
surgery(so4,45,90,45).
...
obtain_better_sol(Room,Day,AgOpRoomBetter,LAgDoctorsBetter,TFinOp):-
		...
		get_time(Tf),
		T is Tf-Ti,
		write('Tempo de geracao da solucao:  '),write(T),nl,nl.
...
update_better_sol(Day,Room,Agenda,LOpCode):-
             ...
             write('NOW: FinTime1  =  '),write(FinTime1),nl,
             write('Agenda  =  '),write(Agenda),nl,nl,
		FinTime1<FinTime,
             write('<----------- Best Solution Updated ----------->'),nl,nl,
