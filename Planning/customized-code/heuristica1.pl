...
doctor_available_for_surgery(Doctor, Day, Duration, (Start, End)) :-
    timetable(Doctor, Day, (StartTime, EndTime)),
    findall((Start, End), (
        agenda_staff(Doctor, Day, Agendas),
        member((S, E, _), Agendas),
        Start >= StartTime,
        End =< EndTime,
        End - Start >= Duration
    ), AvailableSlots),
    sort(2, @=<, AvailableSlots, [(Start, End) | _]).

    
schedule_surgeries_heuristic([], _, _).  % Caso base

schedule_surgeries_heuristic([OpCode | Rest], Room, Day) :-
    surgery_id(OpCode, SurgeryType),
    surgery(SurgeryType, _, SurgeryDuration, _),
    
    find_doctor_earliest(OpCode, Day, SurgeryDuration, Doctor, Slot),
    
    Slot = (Start, End),
    
    retract(scheduled_surgery(Doctor, Day, Slots, Schedule)),
    exclude(inside_slot(Start, End), Slots, UpdatedSlots),
    assertz(scheduled_surgery(Doctor, Day, UpdatedSlots, [(Start, End, OpCode) | Schedule])),

    % Recursivamente agendar as outras cirurgias
    schedule_surgeries_heuristic(Rest, Room, Day).


find_doctor_earliest(OpCode, Day, Duration, Doctor, (Start, End)) :-
    findall((Doctor, Start, End),
        (scheduled_surgery(Doctor, Day, AvailableSlots, _),
         member((Start, End), AvailableSlots),
         End - Start >= Duration), 
        Candidates),
    sort(2, @=<, Candidates, [(Doctor, Start, End) | _]).

inside_slot(Start, End, (S, E)) :- 
    (S >= Start, S =< End); 
    (E >= Start, E =< End).

% Exemplo de consulta para testar a heurística
schedule_with_heuristic(Room, Day, ScheduledSurgeries) :-
    ...
    findall(_,
        (availability(Doctor, Day, AvailableSlots),
         assertz(scheduled_surgery(Doctor, Day, AvailableSlots, []))),
        _),
    
    % Inicia o agendamento das cirurgias
    schedule_surgeries_heuristic(Surgeries, Room, Day),

    % Gera a lista final de agendamentos
    findall((Doctor, Schedule),
        (scheduled_surgery(Doctor, Day, _, Schedule)),
        ScheduledSurgeries).